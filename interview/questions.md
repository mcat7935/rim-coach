...
| 6 (layout) | ✅ 2026-05-16 (first-pass / 一次回答, dedicated session) — paradigm-choice = mille-feuille (AI structure + human position + AI supplement); XY semantic conventions; loop visibility via thick-warm vs thin-cool; loop shape flexible (prominence > shape); density follows maturity; symmetry not required but industry-pattern resemblance aids; font = gothic + loop-color label; RIM ≠ CLD distinction with arrow semantics emerging mid-way (learning required). **Formal RIM notation specification** lives in Flying Penguins' shared drive `G:\共有ドライブ\03_Marketing_Contents\社外発信コンテンツ\RIM` — to be read and integrated separately (background agent dispatched). | `core.md` §18, `layout.md`, `review.md` Check N |
